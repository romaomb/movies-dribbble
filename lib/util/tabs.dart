...
class _CustomTabBarState extends State<CustomTabBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> wrappedTabs = <Widget>[
      for (int i = 0; i < widget.tabs.length + 2; i += 1)
        i.isOdd
            ? widget.customSpacing
            : KeyedSubtree(
                key: _tabKeys[i > 0 ? i~/2 : 0],
                child: widget.tabs[i > 0 ? i~/2 : 0],
              )
    ];
    ...
    for (int index = 0; index < tabCount + 2; index += 1) {
...
class CustomTabBarView extends StatefulWidget {
  ...
  CustomTabBarView({
    ...
    this.dragStartBehavior = DragStartBehavior.start,
        ...
        assert(dragStartBehavior != null),
        this.children = List<Widget>.generate(children.length + 2, (i) => i.isOdd ? Container() : children[i~/2]),
        super(key: key);
